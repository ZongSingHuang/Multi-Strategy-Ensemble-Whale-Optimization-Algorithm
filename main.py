...
for i in range(times):
    ...
    if optimizer.gBest_score>worst: worst = optimizer.gBest_score
    if optimizer.gBest_score<best: best = optimizer.gBest_score
    ...
    x_max = 5.12*np.ones(d)
    x_min = -5.12*np.ones(d)
    optimizer = WOA(fit_func=Noncontinuous_Rastrigin,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
